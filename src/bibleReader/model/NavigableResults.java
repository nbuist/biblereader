...
/**
 * A class that holds results from multiple Bibles and allows getting only some
 * of the results at time. It does so by defining a "window size", which is the
 * number of references that you want to deal with at a time. The methods are
 * not well-documented, but if you spend a little time studying the class, it
 * should be fairly straightforward to see how to use it.
 * 
 * @author Chuck Cusack.
 */
public class NavigableResults {

	private ArrayList<Reference>		results;
	...
	public NavigableResults(ArrayList<Reference> references, String queryPhrase, ResultType type) {
		results = new ArrayList<Reference>(references);
	 ...
	 * 
	 * @return a ArrayList<Reference> containing the currently "viewable" results.
	 */
	public ArrayList<Reference> currentResults() {
		return new ArrayList<Reference>(results.subList(windowStartIndex, windowEndIndex));
	}

	/**
	 * Shift the "viewing window" up and return the next set of results.
	 * 
	 * @return a ArrayList<Reference> containing the currently "viewable" results, after the "viewing window" has been slid up,
	 *         or null if there are no more results.
	 */
	public ArrayList<Reference> nextResults() {
		if (hasNextResults()) {
			slideWindowUp();
	 ...
	 * 
	 * @return a ArrayList<Reference> containing the currently "viewable" results, after the "viewing window" has been slid
	 *         down, or null if there are no previous results.
	 */
	public ArrayList<Reference> previousResults() {
