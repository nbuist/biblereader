...
//	}
	...
	private void renderPage() {
		...
		if (isPassage && !pageRefs.isEmpty()) {
			...
			String header;
			if (first.getBookOfBible().equals(last.getBookOfBible()) && first.getChapter() == last.getChapter()) {
			...
			for (String version : VERSIONS) {
				renderPassageParagraphs(html, pageRefs, version);
			}
		} else {
			...
			for (Reference r : pageRefs) {
				...
				for (String version : VERSIONS) {
					...
					if (!lastSearchText.isBlank()) {
						text = text.replaceAll("(?i)" + java.util.regex.Pattern.quote(lastSearchText), "<b>$0</b>");
					}
	...
	private void renderPassageParagraphs(StringBuilder html, ArrayList<Reference> refs, String version) {
		int lastChapter = -1;
